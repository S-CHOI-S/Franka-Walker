...
log_dir = "../runs/20240806_11-16-38/" # 4 constraints
...
ppo = PPO(N_S, N_A, log_dir, num_avg_constraints=2, avg_cstrnt_limit=[cstrnt1_limit, cstrnt2_limit])
...
for episode_id in range(test_episodes):
    ...
    for _ in range(1000):
        ...
        if state[3] > 1:
            print(f"{RED}state[3]: {RESET}", state[3])
        else:
