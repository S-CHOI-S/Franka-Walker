...
void CController::put_action2_pybind(std::array<double, 4> action)
{
	for (int i = 0; i < 3; i++)
	{
		_drpy(i) = action[i];
	}
	_force_gain = action[3];
}
...
void CController::randomize_env2_pybind(std::array<std::array<double, 3>, 3> rotation_obj, std::string object_name, int scale, std::array<double, 66> pos, double init_theta, double goal_theta, int planning_mode, bool generate_dxyz)
{
	_init_theta = init_theta;
	_goal_theta = goal_theta;
	_object_name = object_name;
...
tuple<vector<double>, vector<double>, float, vector<double>> CController::get_ee_pybind()
{
	x_hand.clear();
	xdot_hand.clear();
	x_circular_hand.clear();
	...
	for (int i = 0; i < 6; i++)
	{
		x_circular_hand.push_back(_x_circular_hand(i));
	}
	gripper_goal = _gripper_goal;
	return make_tuple(xdot_hand, x_hand, gripper_goal, x_circular_hand);
}
...
void CController::control_mujoco()
{
	...
	else if (_control_mode == 4) // circular trajectory RL
	{
		...
		_x_circular_hand.head(3) = _x_des_hand.head(3);
		_x_circular_hand.tail(3) = CircularTrajectory.rotation_circular();
...
void CController::HybridControl()
{
	...
	else if ((target.state == "onvalve_rl") || (target.state == "onvalve_heuristic") || (target.state == "grasp_valve"))
	{
		...
		else
		{
			...
			double force_gain = _force_gain*(13./20. + 7./20.*1000. * (dtheta_des - abs(_dtheta)));
			...
			// cout<<"force gain :"<<_force_gain<<"  Force :"<<_Force.transpose()<<endl;
		}
...
PYBIND11_MODULE(controller, m)
{
	...
	py::class_<CController>(m, "CController")
		...
		.def("randomize_env2", &CController::randomize_env2_pybind)
